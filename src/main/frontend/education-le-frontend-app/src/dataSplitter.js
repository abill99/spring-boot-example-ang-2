export const pluck = (...keys) => (data) => {
  return keys.reduce((subData, key) => {
    return subData === undefined ? subData : subData[key];
  }, data);
};

export const pluck2 = (array, key) => {
  console.log("IN Plucked 2");
  console.log(array.map(o => o[key]));
  console.log("LEAVING Plucked 2");
...
export const toCompanyRatings = (ratings) => {
  ...
  const toChartCategories = (ratings) => {
  ...
  const toGridData = (locality, means) => {
